import { useState, useEffect } from 'react'
import { useParams, useNavigate } from 'react-router-dom'
import { Save, Trash2, ArrowLeft, Bot } from 'lucide-react'
import { useQuery } from '@tanstack/react-query'
import { useAgents } from '../../context/AgentsContext'
import { api } from '../../utils/api'
import type { Agent, AgentFrontmatter } from '../../types'
import { MODEL_IDS, MODEL_META } from '../../utils/models'

const MEMORY_OPTIONS = ['user', 'project', 'local', 'none'] as const

const inputCls = 'w-full px-3 py-2.5 bg-zinc-800 border border-zinc-700 rounded-xl text-sm text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-colors'
const labelCls = 'block text-xs font-medium text-zinc-400 mb-1.5'

export function AgentDetailPage() {
  const { slug } = useParams<{ slug: string }>()
  const navigate = useNavigate()
  const isNew = slug === 'new'

  const { data: agent, isLoading } = useQuery<Agent>({
    queryKey: ['agents', slug],
    queryFn: () => api.get<Agent>(`/agents/${slug}`),
    enabled: !!slug && slug !== 'new',
  })
  const { create, update, remove } = useAgents()

  const [fm, setFm] = useState<Partial<AgentFrontmatter>>({
    name: '', description: '', model: undefined, color: '#3b82f6', memory: undefined,
  })
  const [body, setBody] = useState('')
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    if (agent) {
      setFm(agent.frontmatter)
      setBody(agent.body)
    }
  }, [agent])

  const handleSave = async () => {
    setSaving(true)
    try {
      const payload = { frontmatter: fm as AgentFrontmatter, body }
      if (isNew) {
        const created = await create.mutateAsync(payload)
        navigate(`/agents/${created.slug}`, { replace: true })
      } else {
        await update.mutateAsync({ slug: slug!, payload })
      }
    } finally {
      setSaving(false)
    }
  }

  const handleDelete = async () => {
    if (!confirm(`Delete agent "${slug}"?`)) return
    await remove.mutateAsync(slug!)
    navigate('/agents')
  }

  if (!isNew && isLoading) {
    return (
      <div className="p-6 max-w-3xl">
        <div className="skeleton h-8 w-40 rounded-xl mb-6" />
        <div className="space-y-4">
          <div className="skeleton h-12 rounded-xl" />
          <div className="skeleton h-12 rounded-xl" />
          <div className="skeleton h-64 rounded-xl" />
        </div>
      </div>
    )
  }

  const color = fm.color || '#3b82f6'

  return (
    <div className="p-6 max-w-3xl">
      {/* Header */}
      <div className="flex items-center gap-3 mb-6">
        <button
          onClick={() => navigate('/agents')}
          className="p-1.5 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 rounded-lg transition-colors"
        >
          <ArrowLeft size={16} />
        </button>
        <div className="flex items-center gap-3">
          <div
            className="w-8 h-8 rounded-xl flex items-center justify-center text-white text-sm font-bold shrink-0 shadow-md"
            style={{ background: `linear-gradient(135deg, ${color}, ${color}88)` }}
          >
            {fm.name ? fm.name.charAt(0).toUpperCase() : <Bot size={14} />}
          </div>
          <h1 className="text-xl font-bold text-zinc-100">
            {isNew ? 'New Agent' : fm.name || slug}
          </h1>
        </div>
      </div>

      <div className="space-y-5">
        {/* Name & Color */}
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className={labelCls}>Name</label>
            <input
              type="text"
              value={fm.name || ''}
              onChange={e => setFm(f => ({ ...f, name: e.target.value }))}
              className={inputCls}
              placeholder="My Agent"
            />
          </div>
          <div>
            <label className={labelCls}>Color</label>
            <div className="flex gap-2 items-center">
              <input
                type="color"
                value={fm.color || '#3b82f6'}
                onChange={e => setFm(f => ({ ...f, color: e.target.value }))}
                className="w-10 h-10 rounded-xl cursor-pointer border border-zinc-700 bg-transparent p-1"
              />
              <input
                type="text"
                value={fm.color || '#3b82f6'}
                onChange={e => setFm(f => ({ ...f, color: e.target.value }))}
                className={inputCls}
              />
            </div>
          </div>
        </div>

        <div>
          <label className={labelCls}>Description</label>
          <input
            type="text"
            value={fm.description || ''}
            onChange={e => setFm(f => ({ ...f, description: e.target.value }))}
            className={inputCls}
            placeholder="What this agent does…"
          />
        </div>

        {/* Model & Memory */}
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className={labelCls}>Model</label>
            <select
              value={fm.model || ''}
              onChange={e => setFm(f => ({ ...f, model: e.target.value as typeof fm.model || undefined }))}
              className={inputCls}
            >
              <option value="">Default</option>
              {MODEL_IDS.map(id => (
                <option key={id} value={id}>{MODEL_META[id].label}</option>
              ))}
            </select>
          </div>
          <div>
            <label className={labelCls}>Memory</label>
            <select
              value={fm.memory || ''}
              onChange={e => setFm(f => ({ ...f, memory: e.target.value as typeof fm.memory || undefined }))}
              className={inputCls}
            >
              <option value="">None</option>
              {MEMORY_OPTIONS.map(opt => (
                <option key={opt} value={opt}>{opt}</option>
              ))}
            </select>
          </div>
        </div>

        {/* Instructions */}
        <div>
          <label className={labelCls}>Instructions</label>
          <textarea
            value={body}
            onChange={e => setBody(e.target.value)}
            rows={16}
            className="w-full px-3 py-2.5 bg-zinc-800 border border-zinc-700 rounded-xl text-sm text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 font-mono resize-y transition-colors"
            placeholder="Write agent instructions in markdown…"
          />
        </div>

        {/* Actions */}
        <div className="flex items-center gap-3 pt-2 border-t border-zinc-800/60">
          <button
            onClick={handleSave}
            disabled={saving || !fm.name}
            className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-xl text-sm font-medium transition-colors shadow-lg shadow-blue-900/30"
          >
            <Save size={14} />
            {saving ? 'Saving…' : 'Save'}
          </button>
          {!isNew && (
            <button
              onClick={handleDelete}
              className="flex items-center gap-2 px-4 py-2 bg-zinc-800 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/30 text-zinc-400 border border-zinc-700 rounded-xl text-sm font-medium transition-colors"
            >
              <Trash2 size={14} />
              Delete
            </button>
          )}
        </div>
      </div>
    </div>
  )
}
